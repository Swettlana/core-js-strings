...
function removeFirstOccurrences(str, value) {
  ...
  return pos >= 0 ? str.replace(value, '') : str;
}
/**
...
function removeLastOccurrences(str, value) {
  ...
  return pos >= 0
    ? str.slice(0, pos) + str.slice(pos + value.length, str.length)
    : str;
}
...
function isPalindrome(str) {
  const newStr = str.replace(/[^\w]/g, '').toUpperCase();
...
function reverseWords(str) {
  const arr = str.split(' ');
  const rez = [];
  arr.forEach((el) => rez.push(reverseString(el)));
  return rez.join(' ');
}
...
function invertCase(str) {
  ...
  strArr.forEach((char) => {
    ...
    } else arr.push(char.toLowerCase());
  });
  return arr.join('');
}
...
function extractNameFromTemplate(value) {
  ...
  return value.slice(index + 2, endIndex);
}
...
function encodeToRot13(str) {
  ...
  const lastInd = dic.length - 1;
  const middlePos = dic.length / 2;
  const res = [];
  str.split('').forEach((char) => {
    if (dic.includes(char)) {
      const pos = dic.indexOf(char);
      if (pos >= middlePos) {
        if (pos + 13 > lastInd) {
          res.push(dic[pos + 12 - lastInd + middlePos]);
        } else {
          res.push(dic[pos + 13]);
        }
      }
      if (pos < middlePos) {
        if (pos + 12 >= middlePos - 1) {
          res.push(dic[pos + 13 - middlePos]);
        } else {
          res.push(dic[pos + 13]);
        }
      }
